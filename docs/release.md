...
`rfconversions` is a single Rust crate. The version source is the root `Cargo.toml` package version, currently SemVer. The next version is not inferred from repository evidence, so real releases must pass an explicit version:

```bash
just cut-release --version 0.7.4
```
...
```bash
just cut-release --dry-run --version 0.7.4
```
...
```bash
just cut-release --version 0.7.4 --notes-file path/to/notes.md
```
